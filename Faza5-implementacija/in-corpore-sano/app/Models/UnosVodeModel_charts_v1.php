    ...
    public function getCurrentWeekDataForUser($user_id) {
        ...
        return $this->db->query("
                                    ...
                                    SELECT id_day AS day_of_week, COALESCE((
                                        SELECT SUM(kolicina)
                                        FROM `unos_vode`
                                        WHERE id_kor = {$user_id} AND YEARWEEK(DATE(datum), 1) = YEARWEEK(DATE(NOW()), 1) AND DAYOFWEEK(DATE(datum)) = id_day
                                        GROUP BY DATE(datum)
                                    ), 0) as result
                                    FROM `days_in_week`
    ...
    public function getCurrentMonthDataForUser($user_id) {
        ...
        return $this->db->query("
                                    ...
                                    (
                                        SELECT SUM(kolicina)
                                        FROM `unos_vode`
                                        WHERE id_kor = {$user_id} AND MONTH(NOW()) = MONTH(DATE(datum)) AND DAY(DATE(datum)) = id_day
                                        GROUP BY DATE(datum)
                                    ), 0) as result
                                    FROM `days_in_month`
        ...
        return $this->db->query("
                                    ...
                                    SELECT id_month as month, COALESCE((
                                        ...
                                        FROM (
                                            ...
                                            FROM `unos_vode` 
                                            WHERE id_kor = {$user_id} AND YEAR(NOW()) = YEAR(DATE(datum))
                                            GROUP BY DATE(datum)
                                        ) q
