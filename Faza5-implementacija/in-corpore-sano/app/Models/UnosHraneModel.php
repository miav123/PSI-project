...
class UnosHraneModel {
    ...
    public function getLastWeekDataForUser($user_id) {
        ...
        return $this->db->query("
                                ...
                                WHERE DATE(datum) >= DATE(NOW() - INTERVAL 7 DAY) AND id_kor = {$user_id} AND `obork_sadrzi_namirnice`.id_nam = `namirnica`.id_nam AND `unos_hrane`.`id_obr` = `obork_sadrzi_namirnice`.`id_obr`
                                GROUP BY DATE(datum)
    ...
    public function getLastYearDataForUser($user_id) {
        ...
        return $this->db->query("
                                ...
                                (
                                    ...
                                    FROM `obork_sadrzi_namirnice`, `namirnica`, `unos_hrane`
                                    WHERE DATE(datum) >= DATE(NOW() - INTERVAL 365 DAY) AND id_kor = {$user_id} AND `obork_sadrzi_namirnice`.id_nam = `namirnica`.id_nam AND `unos_hrane`.`id_obr` = `obork_sadrzi_namirnice`.`id_obr`
                                    GROUP BY DATE(datum)
                                ) q
